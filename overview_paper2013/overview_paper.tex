...
\section{Conclusions}
Clojure is a promising candidate for an introductory CS course. Its rich collection of data structures and focus on abstraction teaches students good programming skills. The growing use of Clojure in industry means that there is an active and helpful community surrounding the language and there are a growing number of jobs using it.

Despite the advantages Clojure has, it still poses challenges to its adoption for an introductory class. The primary technical challenges the Clojure poses involve complex errors, a lack of beginner-friendly development environments, and function names that can be misleading. We are working around these problems by creating a template project that will catch 
