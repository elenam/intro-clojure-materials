...
\item Supports concurrency. \joecomment{Add another word to narrow down - C supports concurrency too, but Clojure has much better support}
...
Clojure projects are managed by a tool called {\tt leiningen}. We need to include beginner-friendly error handling and beginner-friendly functions; intercept code typed into REPL. \joecomment{Awkward. Reword (semicolon is out of place)}

We are developing {\tt leiningen} plugins for creating and running student projects (work in progress). \joecomment{*a* leiningen plugin? Presumably not more than one}
\end{frame}
...
Note: {\tt defn} = define function, {\tt fn} = anonymous function (lambda),  {\tt '()} = empty list, {\tt []} = empty vector.  \joecomment{You may want to include some of these earlier - the list/vector syntax in particular will be confusing on the previous slide}
%%{\tiny
\begin{verbatim}
