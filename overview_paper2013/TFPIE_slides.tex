...
\item Designed to supports concurrency. %\joecomment{Add another word to narrow down - C supports concurrency too, but Clojure has much better support}
...
Clojure projects are managed by a tool called {\tt leiningen}. We need to include beginner-friendly error handling and beginner-friendly functions, for program code written in a file and typed into REPL. 
\elenacomment{Better?}
%\joecomment{Awkward. Reword (semicolon is out of place)}

We are developing a {\tt leiningen} plugin for creating and running student projects (work in progress).\joecomment{*a* leiningen plugin? Presumably not more than one} \elenacomment{Actually, possibly more than one, but you are right: we don't need to go into details.}
\end{frame}
