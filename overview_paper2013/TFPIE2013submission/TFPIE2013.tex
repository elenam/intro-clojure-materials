...
that do not appear directly in Clojure (Clojure is implicitly typed) \joecomment{This parenthetical is weird because it's quite a while after the Java types thing. I think you can remove it, esp since you talk about the types stuff down below.} and are not understandable to beginners. 
...
\item for all other errors we rewrite the error message thrown by the Clojure runtime system to use more understandable language for beginners. \joecomment{Rewrote this a bit, probably it should mention trimming the stacktrace too?}
\end{itemize}
...
Unlike lists, vectors are optimized so that adding to the end is more efficient than adding at the beginning \joecomment{This is a little confusing since you earlier mention that vectors are efficient for insertion anywhere. Maybe not an issue, idk}. Consequently, both  \clocode{conj} and \clocode{into} add elements at the end. Calling \clocode{seq} on a vector just returns its elements in order. 
...
Sets correspond to the mathematical notion of a set, and thus are by nature unordered. Iterating over a set via \clocode{seq} often results in elements being ordered by their hash function in the underlying Java implementation \joecomment{I think 'often' should be removed - the way it sounds now is that sets behave randomly}. Consequently, adding elements to a set does not guarantee any specific position of the new element. If the element is already in the set, it will not be added. 
