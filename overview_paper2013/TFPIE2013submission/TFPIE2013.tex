...
There are also cases when simple type checking is not sufficient. For example, even if we check that the first argument to \clocode{map} is a function and the second argument is a sequence, it may be function that is not applicable to the elements of the sequence (such as trying to call \clocode{<} on a string) or expects a wrong number of arguments. 

Below we give examples of some of these situations and the resulting messages. 
