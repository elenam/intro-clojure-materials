...
We argue that an approach to teaching that focuses on Clojure design as it relates to Clojure collections and sequences and focuses on a sequence as an underlying Clojure abstraction provides a solid understanding of Clojure collections and their usage and gives students better techniques for writing programs that are more general and less error-prone. The ideas behind these principles are known in the Clojure community (see 

clarifies the use of 

Clojure provides a rich set of immutable collections. Each collection serves a different purpose, and its  implementation  is typically optimized for efficiency based on its intended use. The four most commonly used Clojure collections are a {\it list}, a {\it vector}, a {\it hash map}, and a {\it set}. Lists are singly-linked lists, similar to Common Lisp lists, vectors are a highly efficient data structure for both insertion and deletion at any position in a logarithmic time, hash maps are collections of key/value pairs, and sets are unordered collections of elements in which every element occurs at most once. 

Each Clojure collection supports three functions that provide a uniform way of interacting with a collection. However, the implementation, and therefore the behavior of each of these functions is different for different types collections. The functions are as follows:
\begin{itemize}
\item {\tt conj} that takes a collection and an item and returns a new collection {\it of the same type} with the new element added to the collection. 
\item {\tt into} that takes a collection and a sequence of elements 
\end{itemize}

The first two columns of table~\ref{table:data-structures} list and give (syntactic) examples of four most commonly used data structures:
