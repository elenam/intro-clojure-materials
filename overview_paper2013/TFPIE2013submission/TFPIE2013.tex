...
%\elenacomment{because they would like dois for their references, and this one is too informal. Ok, I'll add it back.}
...
with an elegant uniform underlying model based on abstraction. Clojure ``collections'' (i.e. data structures) include lists, vectors, sets, and hashmaps. %\joecomment{This is about the 8th time I've seen Clojure referred to as elegant, but the Wikipedia reader in me is screaming "[citation needed]". Just a thought} 
However, 
...
or adjustments to the number of processors. The simplest forms of Clojure multithreading, e.g. a function \clocode{pmap}, which stands for {\it parallel map}, is used exactly the same way as regular \clocode{map}, and is suitable for use in an introductory class. 
%\joecomment{'can be mentioned' sounds weird. How about 'are suitable for use in'?} 
However, explaining principles of parallel execution is beyond the scope of an introductory class. Despite this, background in Clojure would make it easier for students to learn this material in a later course or independently.
...
that do not appear directly in Clojure %(Clojure is implicitly typed) \joecomment{This parenthetical is weird because it's quite a while after the Java types thing. I think you can remove it, esp since you talk about the types stuff down below.} 
and are not understandable to beginners. 
...
Types used in Clojure's implementation form a rich Java hierarchy; for instance, numerous types implement an interface that represents a Clojure sequence type, with subtle differences in internal representations.   
...
\item for all other errors we rewrite the error message thrown by the Clojure runtime system to use more understandable language for beginners and remove a part of the stack trace that refers to anything other than user-developed code. %\joecomment{Rewrote this a bit, probably it should mention trimming the stacktrace too?}
\end{itemize}
...
%\joecomment{A screenshot might be nice here, if there's room}
%\elenacomment{Since our graphics are still under development, I chose not to include screenshots}
\end{enumerate}
Some sample type translations are given in table~\ref{table:types}, and some sample message transformations are in table~\ref{table:messages}. If no type match can be found, either by a lookup or by checking interfaces that a type implements, %(e.g. a variety of types that represent functions that all implement {\tt IFn} interface)
...
A seemingly unrelated problem is confusion about the use of functions \clocode{conj} and \clocode{into}, i.e. functions that add elements to a collection, on predefined Clojure collections. The order in which elements are added to the collection is often not what a programmer expects, there are unexpected {\tt IllegalArgument} errors, and Clojure programmers, especially less experienced ones, often resort to trial and error in order to decide, for example, if they should use a list or a vector (described below).
...
\item \clocode{into} takes a collection and a sequence of elements and adds all of the elements to the collection. \clocode{into} is implemented by adding each element of the collection (in order) using \clocode{conj}. 
\end{itemize}
...
Unlike lists, vectors are optimized so that adding elements at the end is efficient.  
%\joecomment{This is a little confusing since you earlier mention that vectors are efficient for insertion anywhere. Maybe not an issue, idk}. 
Consequently, both  \clocode{conj} and \clocode{into} add elements at the end. Calling \clocode{seq} on a vector just returns its elements in order. 
...
Sets correspond to the mathematical notion of a set, and thus are by nature unordered. Iterating over a set via \clocode{seq} often results in elements being ordered by their hash function in the underlying Java implementation 
%\joecomment{I think 'often' should be removed - the way it sounds now is that sets behave randomly}. 
%\elenacomment{In a way, they do: there are no guarantees about any ordering.}
Consequently, adding elements to a set does not guarantee any specific position of the new element. If the element is already in the set, it will not be added. 
