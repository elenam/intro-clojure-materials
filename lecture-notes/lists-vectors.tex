...
It is quite common in various programming tasks to combine elements of two collections into one. Just as with adding an element to a collection, this can be done in a \fut{collection-specific} or \fut{collection-independent} way. 
...
Another interesting feature of \clocode{concat} is that, unlike \clocode{into}, it takes an arbitrary number of collections (of any type) and concatenates them into one sequence, just like \clocode{+} takes an arbitrary number of numbers and adds them together:
\begin{framed}
\begin{verbatim}
